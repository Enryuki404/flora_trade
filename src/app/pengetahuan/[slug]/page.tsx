...
function mdToHtml(md: string): string {
  ...
  const buildTable = (block: string): string => {
    ...
    const thead =
      headerCells.length > 0
        ? `<thead><tr>${headerCells.map((c) => `<th class="border px-3 py-2 text-left font-bold" style="background: var(--brand-light, #ECFDF5); color: var(--text); border-color: var(--border)">${inline(c)}</th>`).join("")}</tr></thead>`
        : "";
    const tbody =
      bodyRows.length > 0
        ? `<tbody>${bodyRows
            .map((r) => {
              ...
              return `<tr>${cells.map((c) => `<td class="border px-3 py-2" style="color: var(--muted); border-color: var(--border)">${inline(c)}</td>`).join("")}</tr>`;
            })
            .join("")}</tbody>`
        : "";
    return `<div class="overflow-x-auto my-6"><table class="w-full text-sm border-collapse" style="border-color: var(--border)">${thead}${tbody}</table></div>`;
  };
  ...
  for (const block of blocks) {
    ...
    if (/^###\s+/.test(b)) {
      ...
      out.push(`<h3 class="font-bold mt-6 mb-2" style="color: var(--text)">${inline(content)}</h3>`);
      continue;
    }
    if (/^##\s+/.test(b)) {
      const content = b.replace(/^##\s+/, "").trim();
      out.push(`<h2 class="text-xl font-extrabold mt-8 mb-3" style="color: var(--text)">${inline(content)}</h2>`);
      continue;
    }
    ...
    if (/^>\s?/.test(b)) {
      ...
      out.push(`<blockquote class="rounded-r-xl my-4 text-sm p-4" style="border-left:4px solid var(--brand-primary, #0F7A4B); background: var(--brand-light, #ECFDF5); color: var(--muted)">${joined}</blockquote>`);
      continue;
    }
    ...
    if (nonEmptyLines.length > 0 && nonEmptyLines.every((l) => /^\-\s+/.test(l))) {
      const items = nonEmptyLines.map((l) => {
        ...
        return `<li class="ml-6 list-disc" style="color: var(--muted)">${inline(text)}</li>`;
      });
    ...
    if (nonEmptyLines.length > 0 && nonEmptyLines.every((l) => /^\d+\.\s+/.test(l))) {
      const items = nonEmptyLines.map((l) => {
        ...
        return `<li class="ml-6 list-decimal" style="color: var(--muted)">${inline(text)}</li>`;
      });
    ...
    if (para) {
      out.push(`<p class="mt-4 leading-relaxed" style="color: var(--muted)">${inline(para)}</p>`);
    }
  }
...
export default async function ArticleDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="transition-colors duration-300" style={{ backgroundColor: "var(--bg)" }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid lg:grid-cols-[1fr_340px] gap-8 mt-6">
          <article className="rounded-[24px] border overflow-hidden shadow-sm transition-colors duration-300" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
            <div className="relative w-full h-[380px] overflow-hidden" style={{ backgroundColor: "var(--border)" }}>
            ...
            <div className="p-6 lg:p-8">
              <div className="flex flex-wrap items-center gap-2 mb-4">
                ...
                <span className="text-xs transition-colors duration-300" style={{ color: "var(--muted)" }}>{new Date(article.publishedAt).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })} • {article.readingTime}</span>
              </div>
              <h1 className="text-2xl lg:text-3xl font-extrabold leading-tight tracking-tight transition-colors duration-300" style={{ color: "var(--text)" }}>{article.title}</h1>
              <div className="flex items-center gap-3 mt-4 text-sm transition-colors duration-300" style={{ color: "var(--muted)" }}>
                ...
                <span>Ditulis oleh <b style={{ color: "var(--text)" }}>{article.author}</b></span>
              </div>
              <div className="mt-2 flex flex-wrap gap-1.5">
                {article.tags.map((t) => (<span key={t} className="text-xs px-2.5 py-1 rounded-full transition-colors duration-300" style={{ backgroundColor: "var(--brand-light, #ECFDF5)", color: "var(--muted)" }}>{t}</span>))}
              </div>
              ...
              <div className="mt-10 p-5 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 transition-colors duration-300 border" style={{ backgroundColor: "var(--brand-light, #ECFDF5)", borderColor: "color-mix(in srgb, var(--brand-accent, #34D399) 40%, var(--border))" }}>
                <div>
                  <div className="font-bold transition-colors duration-300" style={{ color: "var(--text)" }}>Butuh bantuan ekspor flora ini?</div>
                  <div className="text-sm transition-colors duration-300" style={{ color: "var(--muted)" }}>Konsultasi gratis — kami bantu cek regulasi negara tujuan.</div>
                </div>
                <a
          ...
          <aside className="space-y-6">
            <div className="rounded-2xl border p-6 shadow-sm transition-colors duration-300" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <div className="font-bold transition-colors duration-300" style={{ color: "var(--text)" }}>Artikel Terbaru</div>
                ...
                {latest.map((a) => (
                  <Link key={a.id} href={`/pengetahuan/${a.slug}`} className="flex gap-3 group">
                    ...
                    <div>
                      ...
                      <div className="text-sm font-semibold leading-tight line-clamp-2 transition-colors duration-300 group-hover:text-[var(--brand-primary)]" style={{ color: "var(--text)" }}>{a.title}</div>
                    </div>
                  </Link>
                ))}
              </div>
            ...
            <div className="rounded-2xl border p-6 shadow-sm transition-colors duration-300" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
              <div className="font-bold transition-colors duration-300" style={{ color: "var(--text)" }}>Kategori</div>
            ...
            {related.length > 0 && (
              <div className="rounded-2xl border p-6 shadow-sm transition-colors duration-300" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)" }}>
                <div className="font-bold transition-colors duration-300" style={{ color: "var(--text)" }}>Artikel Terkait</div>
